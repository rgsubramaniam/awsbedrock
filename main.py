...
def getllm_response(llm, vectorstore_faiss, query):
...
def main():
    ...
    with st.sidebar:
        ...
        if st.button("Send"):
            with st.spinner("Processing.."):
               faiss_index = FAISS.load_local("faiss_local", bedrock_embeding, allow_dangerous_deserialization=True) 
               llm = get_llm()
               st.write(getllm_response(llm,faiss_index,  user_question))
